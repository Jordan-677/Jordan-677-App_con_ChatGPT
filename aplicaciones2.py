...
def llenar_vacios_vectorizado(df):
    """Llena los valores vacíos en un DataFrame usando operaciones vectorizadas.

    Args:
        df (pd.DataFrame): El DataFrame con valores vacíos.

    Returns:
        pd.DataFrame: El DataFrame con los valores vacíos llenados.
    """
...
# Opciones del menú
opcion = st.radio(
    "Selecciona una opción", ["Selecciona", "Cargar archivo", "Ingresar URL"]
)

acciones = {
    "Cargar archivo": lambda: "https://raw.githubusercontent.com/gabrielawad/"
    "programacion-para-ingenieria/refs/heads/main/archivos-datos/"
    "aplicaciones/analisis_clientes.csv",
...
try:
    ...
    df_lleno.columns = (
        df_lleno.columns.str.strip().str.replace('á', 'a')
    ...
    correlacion_global = df_lleno[[
        'Edad', 'Ingreso_Anual_USD']].corr()
    ...
    def mostrar_correlacion_segmentada(segmento, variable):
        ...
        correlaciones_segmentadas.columns = [
            segmento, 'Variable', 'Correlacion']
    ...
    df_lleno['Frecuencia_Compra'] = df_lleno['Frecuencia_Compra'].replace(
    ...
    mapa_global = folium.Map(
        location=[df_lleno['Latitud'].mean(), df_lleno['Longitud'].mean()],
        zoom_start=6
    )
    ubicaciones = df_lleno[[
        'Latitud', 'Longitud', 'Género', 'Ingreso_Anual_USD']].dropna().values

    # Uso de numpy para vectorizar la adición de marcadores
    _ = [
        folium.Marker(
            location=[lat, lon], popup=f"{gen}, {ingreso} USD"
        ).add_to(mapa_global) for lat, lon, gen, ingreso in ubicaciones
    ]
    ...
    heat_data = df_lleno[[
        'Latitud', 'Longitud', 'Ingreso_Anual_USD']].values.tolist()
    ...
    kmeans = KMeans(n_clusters=3, random_state=42).fit(
        df_lleno[['Edad', 'Ingreso_Anual_USD']]
    )
    ...
    fig, ax = plt.subplots()
    df_lleno.groupby(['Género', 'Frecuencia_Compra']).size().unstack().plot(
        kind='bar', ax=ax
    )
    st.pyplot(fig)
